# Proposal Funding Crawler — Design Spec

**Status:** Draft  
**Date:** 2025-07-25

## Problem
Dosen/peneliti universitas & politeknik Indonesia kesulitan menemukan proposal
pendanaan riset yang tersebar di puluhan situs kementerian, perusahaan nasional,
dan lembaga internasional. Informasi tidak terpusat, format tidak seragam.

## User Intent
Membangun autonomous agent yang:
- Mencari proposal pendanaan riset dari berbagai sumber
- Mengekstrak data terstruktur (deadline, eligibility, amount)
- Menyimpan ke database terpusat
- Mengirim notifikasi Telegram saat proposal baru ditemukan
...
| F-05 | Deduplikasi via hash(title + url)                   | P0       |
| F-06 | Simpan ke SQLite                                    | P0       |
| F-07 | Notifikasi Telegram                                 | P1       |
| F-08 | Jadwal crawling otomatis (cron)                     | P1       |

### Non-Functional
- Rate limit: 1-3 detik delay, respect robots.txt
- Error recovery: retry 3x, exponential backoff
...
System dipecah jadi 4 unit independent dengan interface yang jelas:
...
Unit D: Data Layer
  ...
  └─ Interface: SQL queries via Hermes/agent tools
  └─ Does: persistence + deduplication
```

### Boundary Rules
- Unit B tidak tahu tentang Unit C/D — hanya terima input, return output terstruktur
...
proposals
  ...
  title         TEXT NOT NULL
  source        TEXT NOT NULL          -- BRIN, LPDP, World Bank
  source_type   TEXT NOT NULL          -- kementerian, nasional, multinasional
  deadline      DATE
  eligibility   TEXT
  field         TEXT                   -- teknologi, sosial, kesehatan
  amount        TEXT                   -- "Rp 500jt"
...
| Pkg Manager    | uv                | Fast, Hermes-compatible                |
